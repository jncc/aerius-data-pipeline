...
def get_background_cells_square(path):
    ...
    ## Concatenate the x+500, y+500, x-500 and y-500 into the points of the gridsquares and turn them into strings
    df_cell['LxLy'] = df_cell['x-500'].astype(str) +' '+ df_cell['y-500'].astype(str)
    df_cell['HxLy'] = df_cell['x+500'].astype(str) +' '+ df_cell['y-500'].astype(str)
    df_cell['HxHy'] = df_cell['x+500'].astype(str) +' '+ df_cell['y+500'].astype(str)
    df_cell['LxHy'] = df_cell['x-500'].astype(str) +' '+ df_cell['y+500'].astype(str)
    #condatenate the centre point of each grid square and turn them into strings
    ##df_cell[['centre']] = df_cell['x'].astype(str) + ' ' + df_cell['y'].astype(str)

    ## Concatenate the center points into
    #df_cell['geometry'] = 'srid=27700;POLYGON((' + df_cell['LxLy'] + ', ' + df_cell['HxLy'] + ', ' + df_cell['HxHy'] + ', ' + df_cell['LxHy'] + ', ' + df_cell['LxLy'] + '))'
    df_cell['geometry'] = 'POLYGON ((' + df_cell['LxLy'] + ', ' + df_cell['HxLy'] + ', ' + df_cell['HxHy'] + ', ' + df_cell['LxHy'] + ', ' + df_cell['LxLy'] + '))'
    
    df_cell = libdb._create_id_col(df_cell, 'background_cell_id')
    df_cell['coords'] = df_cell['x'].astype(str) + ' - ' + df_cell['y'].astype(str)

    return(df_cell[['ukgridcode', 'x', 'y', 'geometry', 'background_cell_id', 'coords']])
...
def get_ammonia_data(path):
    ...
    for file in file_list:
        ...
        for i, row in df_ammonia.iterrows():
            ...
            east = row.easting
            north = row.northing
            #### Create Arrays of Esstings and northings buy subtracting or adding numbers
            eastings = np.array([east-2000, east-1000, east, east+1000, east+2000], dtype=np.intc)
            northings = np.array([north-2000, north-1000, north, north+1000, north+2000], dtype=np.intc)
    ...
    ammonia = df.rename(columns={0: "x", 1: "y", 2: "concentration", 3 : "year"})
...
def get_concentration_data(dirname):
    ...
    # loop through CSV files making changes to them and adding them to a conc_data_list
    for conc_file in files:
        ...
        df_conc_single = df_conc_single.rename(columns={df_conc_single.columns[3]: "concentration"})
...
def create_table_background_concentration(conc_path, ammonia_path):
    ...
    df_conc = pd.merge(df_conc, df_cell[['x', 'y', 'background_cell_id']], how='left', on=['x', 'y'])

    df_ammonia = get_ammonia_data(ammonia_path)

    df_ammonia = pd.merge(df_ammonia, df_cell[['x', 'y', 'background_cell_id']], how='left', on=['x', 'y'])
    ...
    return(df_conc_total[['background_cell_id', 'year', 'substance_id', 'concentration']])
...
def create_table_background_cell_depositions(filepath):
    ...
    df = pd.merge(df, df_cell[['x', 'y', 'background_cell_id']], how='left', on=['x', 'y'])
