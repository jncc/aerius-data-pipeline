...
def _convert_speed_profiles(df_convert):
    ...
    speed_dic = libdb._create_dictionary_from_df(
        df_speed[['speed_profile', 'road_speed_profile_id']]
        )
    df_convert['road_speed_profile_id'] = libdb._convert_id(
...
def _convert_road_type(df_convert):
    ...
    type_dic = libdb._create_dictionary_from_df(
        df_types[['name', 'road_type_category_id']]
        )
    df_convert['road_type'] = libdb._convert_id(
...
def _convert_road_area(df_convert):
    ...
    areas_dic = libdb._create_dictionary_from_df(
...
def _convert_vehicle_type(df_convert):
    ...
    vehicle_dic = libdb._create_dictionary_from_df(
        df_vehicle[['name', 'road_vehicle_category_id']]
        )
    df_convert['vehicle_type'] = libdb._convert_id(
...
def create_table_road_speed_profiles():
    ...
    return(df_out[['road_speed_profile_id', 'speed_limit_enforcement',
...
def create_table_road_category_emission_factors():
    ...
    df_final['substance_id'] = df_final['substance_id'].replace(
                                    'NH3', libdb.get_substance_id('NHx'))

    return(df_final[['road_category_id', 'year', 'substance_id',
...
#     df_cat['description'] = df_cat['vehicle_type']\
#         + '(' + df_cat['country'] + ')'

#     df_cat = libdb._create_id_col(df_cat, 'road_category_id')

#     return(df_cat[['road_category_id', 'gcn_sector_id', 'country',
#                    'vehicle_type', 'name', 'description']])
...
#     df_road_speed['name'] = df_road_speed['road_type'] + ' - '\
#         + df_road_speed['maximum_speed']

#     df_road_speed = libdb._create_id_col(df_road_speed,
#                                          'road_speed_profile_id')

#     return(df_road_speed[['road_speed_profile_id', 'road_type',
#                           'speed_limit_enforcement', 'maximum_speed', 'name']])
...
#     df = pd.merge(
#         df, df_speed[['road_speed_profile_id', 'road_type', 'maximum_speed']],
#         how='left'
#         )
#     df = pd.merge(df, df_cat[['road_category_id', 'country', 'vehicle_type']],
#                   how='left')
...
#     df['substance_id'] = df['substance_id'].replace(
#         'NOx', libdb.get_substance_id('nox')
#         )
#     df['substance_id'] = df['substance_id'].replace(
#         'NH3', libdb.get_substance_id('NHx')
...
#     return(df[[
#         'road_category_id', 'road_speed_profile_id', 'year', 'substance_id',
#         'emission_factor', 'stagnated_emission_factor'
#         ]])
