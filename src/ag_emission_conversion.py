#####   Author: Charlie Howarth/ Kieran Fox
##
##### description: Creating the agricultural emissions tables for 
##### the Aerius air pollution tool
...
housing_cats = ['Housing systems', 'Grazing emissions', 'Outdoor yards (collecting/feeding)']
...
def process_ag_emissions_data(path):
    ...
    df_em = pd.concat([df_housing_em, df_grazing_em, df_yard_em, df_storage_em, df_spreading_em], axis=0)
    ...
    df_mit= pd.concat([df_housing_mit, df_yard_mit, df_storage_mit, df_spreading_mit], axis=0)
...
def create_table_farm_animal_categories(df):
    ...
    return(df_animal[['farm_animal_category_id', 'code', 'name', 'description']])
...
def create_farm_lodging_types(df, animal_names):
    ...
    #df_new['scrubber'] = 'f'
    df_new['scrubber']= df_new.apply(lambda df_new: _assign_scrub(df_new['description']), axis=1)

    # replaceing the animal names with ids and using the dicitonary
    # to keep things consistent
    for name, id in animal_names.items():
        df_new['farm_animal_category_id'] = df_new['farm_animal_category_id'].replace(name, id)
    ...
    return(df_new[['farm_lodging_type_id', 'farm_animal_category_id', 'code', 'name', 'description', 'scrubber']])
...
def create_farm_reductive_lodging_system(df, animal_names):
    ...
    return(df_new[['farm_reductive_lodging_system_id', 'farm_animal_category_id', 'code', 'name', 'description', 'scrubber']])
...
def create_table_farm_reductive_lodging_system_reduction_factor(df):
    ...
    #df['farm_reductive_lodging_system_id'] = df['farm_type'] + ' - ' + df['animal']
    df = df.replace(create_farm_ids_dict(df_mit_total)[1])

    df['reduction_factor'] = df['reduction_factor'] / 100
    
    df['substance_id'] = libdb.get_substance_id('nh3')
    return(df[['farm_reductive_lodging_system_id', 'substance_id', 'reduction_factor']])

def create_table_farm_lodging_types_to_reductive_lodging_systems(df_em, df_mit):
    ...
    mitigations_join['category'] = mitigations_join['source'] + ' - ' + mitigations_join['animal']

    df_links = pd.merge(df_em[['category', 'unique_name']], mitigations_join[['category', 'unique_name']], on='category', how='outer')
    df_links = df_links.rename(columns={'unique_name_x': 'farm_lodging_type_id', 'unique_name_y': 'farm_reductive_lodging_system_id'})
    df_links = df_links.replace(create_farm_ids_dict(df_em)[1])
    df_links = df_links.replace(create_farm_ids_dict(df_mit)[1])

    df_links = df_links[(df_links['farm_lodging_type_id'].notna()) & (df_links['farm_reductive_lodging_system_id'].notna())]
    
    return(df_links[['farm_lodging_type_id', 'farm_reductive_lodging_system_id']])
